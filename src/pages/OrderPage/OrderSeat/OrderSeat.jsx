...
export default function OrderSeat({ branchId, scheduleId, orderSeat, listSeatId, currentListSeat }) {
  ...
  useEffect(() => {
    if (!isSeatsLoaded) {
      ...
      console.log('🚀 ~ OrderSeat ~ currentListSeat:', currentListSeat)
    }
  ...
  useEffect(() => {
    setMyListSeat(currentListSeat)
  }, [currentListSeat])

  const [formats, setFormats] = React.useState(() => [''])
  const handleFormat = (event, newFormats) => {
    console.log('🚀 ~ handleFormat ~ formats:', formats)
    event.preventDefault()
    const selectedSeat = seats.filter((_, index) => newFormats.includes(index))
    const filteredSeatId = selectedSeat.map(seat => seat.id)
    setMyListSeat(removeSameSeat2Arr(myListSeat, filteredSeatId))
    ...
    setTotal(sum)
    orderSeat(filteredSeatId, sum, seats)
  }
  return (
          ...
          <Box sx={{ alignItems: 'center', justifyContent: 'center' }}>
            {rows?.map((row, index) => {
              return <Box key={index} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}>
                ...
                >
                  {row?.map((seat, index1) => {
                    return ((!myListSeat.includes(seat.id)) && (seat.ordered || listSeatId.includes(seat.id)))
                      ? <ToggleButton key={`seat${index1}`} value={(index) * 6 + index1} disabled >
                        <ChairIcon fontSize='large' sx={{ color: '#A0153E' }} />
                      </ToggleButton>
                      : <ToggleButton key={`seat${index1}`} value={(index) * 6 + index1} >
                        <ChairIcon
                          fontSize='large'
                          sx={{
                            color: (formats.includes((index) * 6 + index1))
                              ? '#16FF00'
                              : seat.ordered ? '#A0153E'
                                : ((seat.seatResponse.name.startsWith('B') || seat.seatResponse.name.startsWith('C')) && !myListSeat.includes(seat.id)) ? 'purple'
                                  : 'gray'
                          }}
                        />
                      </ToggleButton>
